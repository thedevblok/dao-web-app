<template v-if="account">
  <div>
    <h1 class="main-title text-center"> VOTE ATTENDANCE FOR VOTE #{{ voteId }} </h1>
    <card
      class="mx-auto my-10 transparent"
      style="max-width: 750px;"
    >
      <div class="pb-4">
        <nuxt-link :to="'/vote/'+voteId" class="btn"
          >Back to vote</nuxt-link
        >
      </div>
      <form
        ...
        <div class="row">
          ...
          <div class="col-12 p-2">
            <base-input
              :label="`Vote amount (${qaoBalance} QAO available)`"
              name="voteAmount"
          ...
          <div class="col-12 p-2 text-center">
            ...
            >
              <span v-if="loading">Submitting.. </span> <span v-else>Submit vote attendance</span>
            </base-button>
          </div>
        </div>
      </form>
    ...
    async createVoteAttendance() {
      try {
        ...
        if(accounts.length > 0) {
          ...
          const gasLimit = await window.qaoVotingEngineContract.methods.vote(
            this.voteId,
          ...
          const voteCompleted = await window.qaoVotingEngineContract.methods.vote(
            this.voteId,
            ethUtils.toWei(this.voteAmount, 'ether'),
            this.lockTime,
            position
