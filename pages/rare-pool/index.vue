        ...
        <div class="col-12 px-4 py-2">
            <card type="default">
                <div class="d-flex align-items-center mb-2">
                    ...
                    ></i>
                    <h3 class="m-0">
                        QAO locked: {{ formattedTotalQaoLocked }} 
                    </h3>
                </div>
            </card>
            <card v-if="nextPeriodWeightData.totalNetworkWeight" type="default">
                <div class="d-flex align-items-center mb-2">
                    ...
                    ></i>
                    <h3 class="m-0">
                        Total Network Weight: {{ formattedTotalWeight }}
                    </h3>
                </div>
                <p>Next payout calculation on {{ nextPeriodWeightData.nextPaymentDate | formatDate }}</p>
            </card>
            <card v-if="nextPeriodWeightData.totalWeightForAddress" type="default">
                <div class="d-flex align-items-center mb-2">
                    ...
                    ></i>
                    <h3 class="m-0">
                        Your Weight: {{ formattedTotalWeightForAddress }} 
                    </h3>
                </div>
                <div class="d-flex align-items-center mb-2">
                    <i
                    ...
                    ></i>
                    <h3 class="m-0">
                        Your Ratio: {{ formattedWeightRatio }} %
                    </h3>
                </div>
            </card>
            <card>
                <h4 class="card-title">
                    Percentage of locked QAO for the next 520 weeks based on
                    current amount
                </h4>
                <line-chart
                    style="height: 100%"
...
export default {
    ...
    computed: {
        ...
        formattedWeightRatio() {
          const ratio = this.nextPeriodWeightData.totalWeightForAddress / this.nextPeriodWeightData.totalNetworkWeight * 100
        ...
        formattedTotalWeightForAddress() {
          return formatCryptoAmount(this.nextPeriodWeightData.totalWeightForAddress);
        },
        ...mapGetters({
          account: 'getAccount'
        })
            ...
            blueBarChart: {
                ...
                chartData: {
                    ...
                    datasets: [
                        {
                            type: 'line',
                            label: 'Percentage of locked QAO',
                            fill: true,
    ...
    methods: {
      async getNextPeriodTotalWeight() {
        ...
        this.nextPeriodWeightData = await this.$axios.$get('/api/get-next-period-total-weight', {params: { address }})
