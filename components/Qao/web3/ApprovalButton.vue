<template>
<div>
  <p>Once approving, please wait for the process to finish on the blockchain before quitting this page.</p>
  <v-btn id="approve-btn" class="qao-secondary-btn btn" :class="{'disabled': loading}" @click="approveQao">
    <span v-if="loading">Approving... </span> <span v-else>Approve QAO</span>
  </v-btn>
</div>
</template>
      ...
      try {
        ...
        if (accounts.length > 0) {
          ...
          if (qaoApprovalLimit.status) {
            this.$store.dispatch('triggerQaoApprovalLimit', 2000000000000000000000000000000)
          }
        }
      } catch (e) {
        this.loading = false
