  ...
  <div class="vote-list-holder">
    <div class="row mx-auto text-center" style="max-width: 760px;">
      ...
      <div class="col-12 col-sm-6 mb-2 text-sm-left">
        <nuxt-link to="#" class="btn-info btn disabled" disabled>Create vote</nuxt-link>
        <p class="red-text">Coming soon</p>
      </div>
      <div class="col-12 col-sm-6 mb-2 text-sm-right">
        <nuxt-link to="/vote/attendance" class="btn-info btn">Withdraw vote attendances</nuxt-link>
      </div>
    </div>
    <div class="row mx-auto text-center" style="max-width: 760px;">
      <div class="col-12">
              ...
              <card>
                <div class="content">
                  <h4>
                    ...
                    <i
                      class="tim-icons "
                      :class="{ 'icon-check-2': vote.votePositive > vote.voteNegative, 'icon-simple-remove': vote.votePositive < vote.voteNegative }"
                    ></i>
                  </h4>
                  <p>
                    {{ vote.description }}
                  </p>
                  <nuxt-link class="btn btn-info btn-simple" :to="`/vote/${vote.voteId}`"> VIEW MORE </nuxt-link>
                </div>
              </card>
            </div>
          </div>
...
export default {
  ...
  async mounted() {
    ...
    currentVotes.forEach((element) => {
      const currentDescription = element.description.length > 100 ? element.description.substring(0, 100) + "..." : element.description
    ...
    previousVotes.forEach((element) => {
      const previousDescription = element.description.length > 100 ? element.description.substring(0, 100) + "..." : element.description
