<template>
  <div class="vote-list-holder">
    ...
    <div class="row mx-auto text-center" style="max-width: 760px;">
      <div class="col-12">
          <h3 class="sub-title mt-4">You can collect your QAO that have finished their staking period here.</h3>
            ...
            <el-table :data="voteAttendances">
              ...
              <template v-slot:default="table">
                <a
                  ...
                  :disabled="loading"
                >
                  <span v-if="loading === false">Withdraw</span><span v-else>Withdrawing...</span>
                </a>
              </template>
              </el-table-column>
            </el-table>
  ...
  export default {
    ...
    methods: {
      async withdrawFromAttendance(attendanceId) {
        ...
        try {
          ...
          if(accounts.length > 0) {
            ...
            const gasLimit = await window.qaoVotingEngineContract.methods.withdrawFromVoteAttendance(
              attendanceId
            ).estimateGas(
              {
                from: account,
            ...
            const voteWithdrawn = await window.qaoVotingEngineContract.methods.withdrawFromVoteAttendance(
              attendanceId
            ).send(
              {
                from: account,
      ...
      async getVoteAttendances() {
        if (this.currentAccount) {
          const currentVoteAttendances = await this.$axios.$get('/api/get-mature-vote-attendances', {
            params: {
